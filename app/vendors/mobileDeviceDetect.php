<?php
function _mobile_device_detect($iphone = true, $ipad = true, $android = true, $opera = true, $blackberry = true, $palm = true, $windows = true, $mobileredirect = false, $desktopredirect = false)
    {
        $mobile_browser = false; // set mobile browser as false till we can prove otherwise
        $user_agent = $_SERVER['HTTP_USER_AGENT']; // get the user agent value - this should be cleaned to ensure no nefarious input gets executed
        $accept = !empty($_SERVER['HTTP_ACCEPT']) ? $_SERVER['HTTP_ACCEPT'] : ''; // get the content accept value - this should be cleaned to ensure no nefarious input gets executed
...
if ($mobile_browser == true) {    
	  ...
	  if('m.' != substr($server_name, 0, 2)){
		 $url = 'http://m.'.$server_name;
		 header('Location: ' . $url); // redirect to the right url for this device
		 exit;
	  }
    ...
    if ($mobile_browser == '') {
        ...
        return array(
            $mobile_browser,
            $status
        ); // is a mobile so we are returning an array ['0'] is true ['1'] is the $status value
