    ...
    function update_affiliate_status()
    {
        ...
        $affiliates = $this->find('all', array(
            ...
            'fields' => array(
                ...
                'DATEDIFF(now(), Affiliate.commission_holding_start_date) as expected_date_diff'
            ) ,
            'recursive' => - 1,
        ));
        foreach($affiliates as $affiliate) {
            if (($affiliate['0']['expected_date_diff'] != '') && ($affiliate['0']['expected_date_diff'] >= Configure::read('affiliate.commission_holding_period'))) {
				...
				if (!empty($affiliate['Affiliate']['commission_amount'])) {
					App::import('Model', 'Transaction');
					$this->Transaction = new Transaction();
					$data = array();
					$data['Transaction']['user_id'] = $affiliate['Affiliate']['affliate_user_id'];
					$data['Transaction']['foreign_id'] = ConstUserIds::Admin;
					$data['Transaction']['class'] = 'SecondUser';
					$data['Transaction']['amount'] = $affiliate['Affiliate']['commission_amount'];
					$data['Transaction']['transaction_type_id'] = ConstTransactionTypes::AmountTakenForAffiliate;
					$this->Transaction->log($data);
					$data = array();
					$data['Transaction']['user_id'] = ConstUserIds::Admin;
					$data['Transaction']['foreign_id'] = $affiliate['Affiliate']['affliate_user_id'];
					$data['Transaction']['class'] = 'User';
					$data['Transaction']['amount'] = $affiliate['Affiliate']['commission_amount'];
					$data['Transaction']['transaction_type_id'] = ConstTransactionTypes::AmountTakenForAffiliateFromAdmin;
					$this->Transaction->log($data);
				}
            }
        }
