   ...
   public $hasAndBelongsToMany = array(
    ...
    function __construct($id = false, $table = null, $ds = null)
    {
        ...
        $this->moreActions = array(
            ...
            //ConstMoreAction::EnableMerchantProfile => __l('Enable Merchant Profile')
        );
    ...
    {
        if ($user_type != ConstUserTypes::Merchant || ($user_type == ConstUserTypes::Merchant && Configure::read('user.is_merchant_actas_normal_user'))) {
            return true;
        }
        return false;
    }
    ...
    function _createCimPaymentProfile($data)
    {
        ...
        if (!empty($cim)) {
            ...
            $cim->setParameter('billToMerchant', Configure::read('site.name'));
    ...
    function _updateCimPaymentProfile($data)
    {
        ...
        if (!empty($cim)) {
            $cim->setParameter('refId', time());
            $cim->setParameter('merchant', Configure::read('site.name'));
            $cim->setParameter('customerProfileId', $data['customerProfileId']);
    ...
    function _createCustomerProfileTransaction($data, $type)
    {
        ...
        if (!empty($cim)) {
            ...
            if ($type == 'profileTransAuthOnly') {
                $title = Configure::read('site.name') . ' - Item Amount Authorize';
                $description = 'Authorize item amount in ' . Configure::read('site.name');
            } else {
            ...
            $cim->setLineItem($data['item_id'], $title, $description, $data['quantity'], $unit_amount);
            $cim->createCustomerProfileTransaction($type);
            $response = $cim->getDirectResponse();
            $response_array = explode(',', $response);
            $data_authorize_docapture_log['AuthorizenetDocaptureLog']['item_user_id'] = $data['item_id'];
            ...
            $get_authorize_id = $this->ItemUser->AuthorizenetDocaptureLog->getLastInsertId();
            if ($cim->isSuccessful()) {
