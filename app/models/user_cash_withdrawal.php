...
class UserCashWithdrawal extends AppModel
{
    ...
    function _checkAmount($amount)
    {
        ...
        } else if ($this->data[$this->name]['user_type_id'] == ConstUserTypes::Merchant) {
            if (($amount < Configure::read('merchant.minimum_withdraw_amount')) || ($amount > Configure::read('merchant.maximum_withdraw_amount'))) {
                $this->validationErrors['amount'] = sprintf(__l('Given amount should lies from  %s%s to %s%s') , Configure::read('site.currency') , Configure::read('merchant.minimum_withdraw_amount') , Configure::read('site.currency') , Configure::read('merchant.maximum_withdraw_amount'));
            }
        }
        return false;
    }
	function _automaticTransferAmount($user_type_id){
        ...
        } elseif ($user_type_id == ConstUserTypes::Merchant) {
