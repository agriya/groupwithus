...
class UserCashWithdrawalsController extends AppController
{
    ...
    {		
        ...
        if (!Configure::read('merchant.is_user_can_withdraw_amount') && $this->Auth->user('user_type_id') == ConstUserTypes::Merchant) {
            throw new NotFoundException(__l('Invalid request'));
        }
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->UserCashWithdrawal->validates()) {
                ...
                if ($this->UserCashWithdrawal->save($this->request->data)) {
					...
					$ajax_url = Router::url(array('controller' => 'user_cash_withdrawals','action' => 'index'));
    ...
    public function admin_index()
    {
		...
		if (!empty($this->request->data['UserCashWithdrawal']['account_id']) && $this->request->data['UserCashWithdrawal']['account_id'] != 'all') {            
            $userCashWithdrawals = $this->UserCashWithdrawal->find('all', array(
                'conditions' => array(
                    'UserCashWithdrawal.withdrawal_status_id' => $this->request->data['UserCashWithdrawal']['filter_id'],
                ) ,
				'contain' => array(
					'User' => array(
	...
	public function admin_move_to($id = null){
		...
		$massPaylogTables = array(ConstPaymentGateways::PayPalAuth => 'PaypalTransactionLog');
