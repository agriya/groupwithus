...
class UserFriendsController extends AppController
{
    ...
    public function myfriends($user_id = null, $status = null)
    {
        ...
        if (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'item')) {
            $user_id = $this->Auth->user('id');
            $status = ConstUserFriendStatus::Approved;
            if (!empty($this->request->params['named']['type']) && !empty($this->request->params['named']['item'])) {
                $this->request->data['UserFriend']['item_slug'] = $this->request->params['named']['item'];
            }
        }
    ...
    public function add($username = null)
    {
        ...
        if ($this->Auth->user('user_type_id') == ConstUserTypes::Merchant && !Configure::read('user.is_merchant_actas_normal_user')) {
            throw new NotFoundException(__l('Invalid request'));
        }
        ...
        if (!$my_self) {
            ...
            if ($blockedUser) {
                $this->Session->setFlash(sprintf(__l('%s is blocked you. You can not become friend of %s') , $user['User']['username'], $user['User']['username']) , 'default', null, 'error');
                $this->redirect(array(
                    'controller' => 'users',
                    'action' => 'view',
                    $user['User']['username']
            ...
            // Check user is a merchant user
            $checkMerchantUser = $this->UserFriend->User->find('first', array(
                'conditions' => array(
                    'User.id' => $user['User']['id'],
                ) ,
                'recursive' => -1
            ));
            if ($checkMerchantUser['User']['user_type_id'] == ConstUserTypes::Merchant && !Configure::read('user.is_merchant_actas_normal_user')) {
            ...
            // friend settings
            $userSettings = $this->UserFriend->User->UserProfile->find('first', array(
                'conditions' => array(
                    'UserProfile.user_id' => $user['User']['id'],
                ) ,
                'fields' => array(
                    'UserProfile.is_auto_approve_friend_request'
                ) ,
                'recursive' => -1
            ));
            if (empty($userFriend)) {
                $friend_status = (!empty($userSettings['UserProfile']['is_auto_approve_friend_request']) && $userSettings['UserProfile']['is_auto_approve_friend_request']) ? (ConstUserFriendStatus::Approved) : (ConstUserFriendStatus::Pending);
                ...
                if ($this->UserFriend->save($this->request->data)) {
					...
					$userNotifications = $this->UserFriend->User->UserNotification->find('first', array(
						'conditions' => array(
                    ...
                    if (Configure::read('friend.is_send_email_on_friend_request') && $userNotifications['UserNotification']['when_user_follow_or_request_to_follow_me']) {
                    ...
                    if ($userSettings['UserProfile']['is_auto_approve_friend_request'] and Configure::read('friend.is_two_way')) {
    ...
    public function admin_update()
    {
        if (!empty($this->request->data['UserFriend'])) {
            ...
            if ($actionid && !empty($userFriendIds) && !empty($actionid)) {
                ...
                $this->Session->setFlash(sprintf(__l('Checked users has been %s') , $status) , 'default', null, 'success');
            }
        }
    ...
    public function import($domain = '')
    {
        ...
        if (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'item') && !empty($this->request->params['named']['item'])) {
            $this->Session->write('item_slug', $this->request->params['named']['item']);
        }
        if (empty($this->request->params['named']['type']) && $this->Session->check('item_slug')) {
            $this->Session->delete('item_slug');
        }
        $item_slug_check = $this->Session->read('item_slug');
        if (!empty($item_slug_check)) {
            //	echo $item_slug_check;
            $this->request->data['UserFriend']['item_slug'] = $item_slug_check;
            //$this->Session->delete('item_slug');
    ...
    public function importfriends()
    {
        ...
        if (!empty($this->request->data)) {
            foreach($this->request->data['UserFriend']['FriendList'] as $temp_id => $option) {
                if ($option == 1) {
                    ...
                    if ($temp_contact['TempContact']['is_member'] == 1) {
                        ...
                        if (!empty($this->request->data['UserFriend']['item_slug'])) {
                            $user_invited[] = array(
                                'User' => array(
            ...
            if (!empty($this->request->data['UserFriend']['item_slug']) && !empty($user_invited)) {
                $this->UserFriend->inviteItemUsers($user_invited, $this->request->data['UserFriend']['item_slug']);
            }
    ...
    public function _sendInviteMail($contact_email)
    {
        ...
        $email_replace = array(
            ...
            '##USERNAME##' => $this->Auth->user('username') ,
            '##SITE_NAME##' => Configure::read('site.name') ,
            '##SITE_URL##' => Router::url(array(
                'controller' => 'users',
                'action' => 'register',
				'r' => $this->Auth->user('username'),
                'admin' => false
            ) , true) ,
    ...
    public function item_invite()
    {
        $this->pageTitle = __l('Invite your friends for the item');
        if (!Configure::read('Item.invite_after_item_add')) {
            throw new NotFoundException(__l('Invalid request'));
        }
        if (!empty($this->request->data)) {
            $ids = array();
        ...
        if (!empty($this->request->params['named']['item'])) {
            $item = $this->UserFriend->User->ItemUser->Item->find('first', array(
                'conditions' => array(
            ...
            $this->set('item', $item);
            $this->set('item_slug', $this->request->params['named']['item']);
