...
class UsersController extends AppController
{
    ...
    public function view($username = null)
    {
        ...
        $statistics['item_purchased'] = $this->User->ItemUser->find('count', array(
            'conditions' => array(
                'ItemUser.user_id' => $user['User']['id'],
                'ItemUser.is_gift' => 0
            ) ,
            'recursive' => -1
        ));
    ...
    public function register($type = null)
    {
        ...
        if (empty($this->request->data)) {
            if (!empty($twuser)) {
                ...
                $this->request->data['User']['twitter_username'] = $twuser['username'];
                $this->request->data['User']['email'] = '';
        ...
        if (!empty($openid['openid_url'])) {
            if (isset($openid['email'])) {
                ...
                if (!empty($user_type_check) && $user_type_check == 'merchant') {
                    $this->request->data['User']['type'] = $type = 'merchant';
                }
        ...
        if (!empty($user_type_check) && $user_type_check == 'merchant') {
            $this->request->data['Merchant']['name'] = '';
            $this->request->data['Merchant']['address1'] = '';
            $this->request->data['Merchant']['zip'] = '';
            ...
            if (!empty($this->request->data)) {
				if (empty($type)) {
					if (!empty($_COOKIE['_geo'])) {
						...
						$this->request->data['Merchant']['latitude'] = $_geo[3];
						$this->request->data['Merchant']['longitude'] = $_geo[4];
					}
				}
                ...
                if (!empty($this->request->data['City']['name'])) {
                    $this->request->data['UserProfile']['city_id'] = !empty($this->request->data['City']['id']) ? $this->request->data['City']['id'] : $this->User->Merchant->City->findOrSaveAndGetId($this->request->data['City']['name']);
                }
                if (!empty($this->request->data['State']['name'])) {
					$this->request->data['UserProfile']['state_id'] = !empty($this->request->data['State']['id']) ? $this->request->data['State']['id'] : $this->User->Merchant->State->findOrSaveAndGetId($this->request->data['State']['name']);
                }
                ...
                $this->User->Merchant->set($this->request->data);
                if (!empty($this->request->data['User']['type'])) {
                    $this->User->Merchant->City->set($this->request->data);
                    $this->User->Merchant->State->set($this->request->data);
                }
                if ($this->User->validates() &$this->User->UserProfile->validates() &$this->User->Merchant->validates() &$this->User->Merchant->City->validates() &$this->User->Merchant->State->validates()) {
                    $this->User->create();
					...
					if (!empty($this->request->data['User']['is_facebook_register'])) {
						$this->request->data['User']['profile_image_id'] = ConstProfileImage::Facebook;
					} elseif (!empty($this->request->data['User']['is_twitter_register'])) {
						$this->request->data['User']['profile_image_id'] = ConstProfileImage::Twitter;
					}
                    ...
                    if ($this->User->save($this->request->data, false)) {
						if (!empty($this->request->data['User']['referred_by_user_id'])) {
							$this->_sendMailToReferredUser($this->request->data['User']['referred_by_user_id'], $this->User->getLastInsertId());
						}
                        $this->User->_createCimProfile($this->User->getLastInsertId());
                        if (!empty($type)) {
                            if (!empty($this->request->data['City']['name'])) {
                                $this->request->data['UserProfile']['city_id'] = $this->request->data['Merchant']['city_id'] = !empty($this->request->data['City']['id']) ? $this->request->data['City']['id'] : $this->User->Merchant->City->findOrSaveAndGetId($this->request->data['City']['name']);
                            }
                            if (!empty($this->request->data['State']['name'])) {
								$this->request->data['UserProfile']['state_id'] = $this->request->data['Merchant']['state_id'] = !empty($this->request->data['State']['id']) ? $this->request->data['State']['id'] : $this->User->Merchant->State->findOrSaveAndGetId($this->request->data['State']['name']);
							}
                            $this->request->data['Merchant']['user_id'] = $this->User->getLastInsertId();
                            $this->request->data['Merchant']['is_online_account'] = 1;
							$this->request->data['Merchant']['is_merchant_profile_enabled'] = 1;
                            $this->User->Merchant->create();
                            $this->User->Merchant->save($this->request->data['Merchant']);
                            $merchant_id = $this->User->Merchant->getLastInsertId();
                        }
                        ...
                        $this->request->data['UserNotification']['user_id'] = $this->User->id;
                        $this->User->UserNotification->create();
                        $this->User->UserNotification->save($this->request->data);
                        if (Configure::read('user.is_admin_mail_after_register')) {
                            ...
                            $emailFindReplace = array(
                                ...
                                '##SIGNUP_IP##' => RequestHandlerComponent::getClientIP(),
                        ...
                        if (!empty($this->request->data['User']['openid_url']) || !empty($this->request->data['User']['fb_user_id'])) {
                            ...
                            if ($this->Auth->login($this->request->data)) {
                                ...
                                if ($this->Auth->user('user_type_id') == ConstUserTypes::Merchant) {
                                    $merchant = $this->User->Merchant->find('first', array(
                                        'conditions' => array(
                                            'Merchant.user_id = ' => $merchant_id
                                        ) ,
                                        'fields' => array(
                                            'Merchant.slug',
                                        ) ,
                        ...
                        if (!Configure::read('user.is_email_verification_for_register') and Configure::read('user.is_auto_login_after_register')) {
                            ...
                            if ($this->Auth->login($this->request->data)) {
                                ...
                                if ($this->request->data['User']['user_type_id'] == ConstUserTypes::Merchant) {
                                    $merchant = $this->User->Merchant->find('first', array(
                                        'conditions' => array(
        ...
        unset($this->User->Merchant->City->validate['City']);
    ...
    public function profile_image($id = null)
    {
        ...
        if (is_null($id) or ($this->Auth->user('user_type_id') != ConstUserTypes::Admin && $id != $this->Auth->user('id'))) {
            throw new NotFoundException(__l('Invalid request'));
        }
        $user = $this->User->find('first', array(
            'conditions' => array(
        ...
        $profileimages = array(
            ConstProfileImage::Twitter => "Twitter",
            ConstProfileImage::Facebook => "Facebook",
            ConstProfileImage::Upload => "Upload"
        );
    ...
    public function admin_export($hash = null)
    {
        ...
        if (!empty($this->request->params['named']['main_filter_id'])) {
            if ($this->request->params['named']['main_filter_id'] == ConstMoreAction::OpenID) {
                ...
                $conditions['User.user_type_id != '] = ConstUserTypes::Merchant;
    ...
    {
		if (Configure::read('referral.referral_enabled_option') == ConstReferralOption::Disabled) {
    ...
    public function _sendMailToReferredUser($referred_by_user_id, $user_id)
	{
		if (!empty($referred_by_user_id) && !empty($user_id)) {
			...
			$userNotification = $this->User->UserNotification->find('first', array(
				'conditions' => array(
					'UserNotification.user_id' => $referred_by_user_id,
					'UserNotification.when_user_registered_from_my_invitation' => 1,
				) ,
				'contain' => array(
					'User' => array(
						'fields' => array(
			...
			if (!empty($userNotification) && !empty($user)) {
				$this->loadModel('EmailTemplate');
				$email = $this->EmailTemplate->selectTemplate('Referrer Registered Mail');
				$emailFindReplace = array(
					'##SITE_URL##' => Router::url('/', true),
					'##SITE_NAME##' => Configure::read('site.name'),
					'##USERNAME##' => $userNotification['User']['username'],
					'##FRIEND_USERNAME##' => $user['User']['username'],
					'##USER_LINK##' => Router::url(array(
						'controller' => 'users',
						'action' => 'view',
						$user['User']['username'],
        ...
        if (!$this->User->isValidActivateHash($user_id, $hash)) {
            ...
            if (Configure::read('user.is_auto_login_after_register')) {
                ...
                if ($this->Auth->login($this->request->data)) {
                    ...
                    $this->User->UserLogin->insertUserLogin($this->Auth->user('id'));
                    if ($user['User']['user_type_id'] == ConstUserTypes::Merchant) {
                        if (!Configure::read('user.is_merchant_actas_normal_user')) {
                            $merchant = $this->User->Merchant->find('first', array(
                                'conditions' => array(
    ...
    public function resend_activation($user_id = null, $hash = null)
    {
        ...
        if ($this->Auth->user('user_type_id') == ConstUserTypes::Admin) {
            $this->redirect(array(
                'controller' => (!empty($this->request->params['named']['type'])) ? 'merchants' : 'users',
                'action' => 'index',
                'admin' => true
            ));
        } else {
    ...
    public function _facebook_login($id = null)
    {
        ...
        if (empty($user)) {
            ...
            //end
            // Redirect to registeration for merchant users to fill other details //
            $user_type_check = $this->Session->read('user_type');
            if (!empty($user_type_check) && $user_type_check == 'merchant') {
            ...
            if ($this->Auth->login($this->request->data)) {
                ...
                if ($redirectUrl = $this->Session->read('Auth.redirectUrl')) {
                    ...
                    if (!empty($user['User']['user_type_id']) && ($user['User']['user_type_id'] == ConstUserTypes::Merchant)) {
                        $this->redirect(array(
                            'controller' => 'merchants',
                            'action' => 'dashboard',
                        ));
    ...
    public function login($username = null)
    {
        ...
        isset($this->request->params['named']['id']) ? $temp['User']['item_id'] = $this->request->params['named']['id'] : $temp['User']['item_id'] = '';
        ...
        if (!empty($this->request->params['named']['user_type']) && $this->request->params['named']['user_type'] == 'merchant') {
            $this->Session->write('user_type', 'merchant');
        } else {
        ...
        if (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'openid' && Configure::read('user.is_enable_openid')) {
	...
	public function fs_oauth_callback()
	{
		...
		if (array_key_exists('code', $_GET)) {
			...
			$fs_user_lastname = $user->response->user->lastName;
    ...
    public function change_password($user_id = null)
    {
        ...
        $this->loadModel('EmailTemplate');
        if (($this->Auth->user('user_type_id') == ConstUserTypes::Merchant) || ($this->Auth->user('user_type_id') == ConstUserTypes::User)) {
    ...
    public function admin_index()
    {
		...
		$conditions['User.user_type_id != '] = ConstUserTypes::Merchant;
		$count_conditions['User.user_type_id != '] = ConstUserTypes::Merchant;
        if (!empty($this->request->params['named']['filter_id'])) {
            if ($this->request->params['named']['filter_id'] == ConstMoreAction::Active) {
                $conditions['User.is_active'] = 1;
                $this->pageTitle.= __l(' - Active ');
    ...
    public function admin_add()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->User->save($this->request->data) &$this->User->UserProfile->validates()) {
                ...
                $this->User->UserProfile->create();
                $this->User->UserProfile->save($this->request->data);
                $this->request->data['UserNotification']['user_id'] = $this->User->getLastInsertId();
                $this->User->UserNotification->create();
                $this->User->UserNotification->save($this->request->data);
        ...
        $userTypes = $this->User->UserType->find('list', array(
            'conditions' => array(
                'UserType.id !=' => ConstUserTypes::Merchant
            )
        ));
        ...
        if (!empty($this->request->data['User'])) {
            ...
            if ($actionid && !empty($userIds)) {
                if ($actionid == ConstMoreAction::Inactive) {
                    ...
                    exit;
                } else if ($actionid == ConstMoreAction::Displayhome) {
                    $this->User->updateAll(array(
                        'User.is_home' => 1,
                    ) , array(
                        'User.id' => $userIds
                    ));
                    $this->Session->setFlash(__l('Checked users has been set to display in home') , 'default', null, 'success');
                } else if ($actionid == ConstMoreAction::NotDisplayhome) {
                    $this->User->updateAll(array(
                        'User.is_home' => 0,
                    ) , array(
                        'User.id' => $userIds
                    ));
                    $this->Session->setFlash(__l('Checked users has been unset to display in home') , 'default', null, 'success');
                } else if ($actionid == ConstMoreAction::EnableMerchantProfile) {
					$this->User->Merchant->updateAll(array(
						'Merchant.is_merchant_profile_enabled' => 1
					) , array(
						'Merchant.user_id' => $userIds
					));
    ...
    {
        $this->setAction('register', 'merchant');
    }
    public function add_to_wallet()
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->User->validates()) {
                if ($this->request->data['User']['payment_gateway_id'] == ConstPaymentGateways::CreditCard) {
                    ...
                    if ($paymentGateway['PaymentGateway']['name'] == 'PayPal') {
                        ...
                        $gateway_options['init'] = array(
                            ...
                            'format' => array(
                                ...
                                'item_quant' => '1',
                                'item_valor' => $this->request->data['User']['amount'],
                                'item_frete' => '0',
                                'item_peso' => '20'
                            ) ,
                            'customer_info'
                        ...
                        $transaction_data['TempPaymentLog'] = array(
                            ...
                            'payment_type' => 'wallet',
                            'user_id' => $this->Auth->user('id') ,
                            'ip_id' => $this->User->toSaveIp(),
                            'amount_needed' => $this->request->data['User']['amount'],
                    ...
                    $this->set('amount', $this->request->data['User']['amount']);
        ...
        $gateway_options['countries'] = $this->User->UserProfile->City->Country->find('list', array(
            'fields' => array(
                'Country.iso2',
                'Country.name'
            ) ,
        ...
        $gateway_options['states'] = $this->User->UserProfile->City->State->find('list', array(
            'conditions' => array(
                'State.is_approved =' => 1
            ) ,
            'fields' => array(
        ...
        $states = $this->User->UserProfile->City->State->find('list', array(
            'conditions' => array(
                'State.is_approved =' => 1
            ) ,
            'fields' => array(
        ...
        if (!empty($this->request->data)) {
            ...
            if (!empty($cim)) {
                ...
                if (!empty($approval_code) && !empty($response)) {
                    ...
                    $this->User->ItemUser->AuthorizenetDocaptureLog->save($data_authorize_docapture_log);
    ...
    public function _addWalletFromCreditCard($data)
    {
        ...
        $this->User->ItemUser->PaypalDocaptureLog->save($data_paypal_docapture_log);
        $this->Session->setFlash(__l('Your payment has been successfully transferred.') , 'default', null, 'success');
        $this->redirect(array(
            'controller' => 'users',
            'action' => 'my_stuff',
    ...
    public function my_stuff()
    {
        ...
        $userNotifications = $this->User->UserNotification->find('all', array(
            'conditions' => array(
                'User.id =' => $this->Auth->user('id')
            )
        ));
        $this->set('userNotifications', $userNotifications);
    }
    public function referred_users()
    {
        ...
        $referred_users_item_counts = $this->User->ItemUser->find('all', array(
            'conditions' => array(
                'ItemUser.user_id' => $userlist
            ) ,
            'fields' => array(
                'Count(ItemUser.user_id) as item_count',
                'ItemUser.user_id'
            ) ,
            'group' => array(
                'ItemUser.user_id'
        ...
        foreach($referred_users_item_counts as $referred_users_item_count) $new_count[$referred_users_item_count['ItemUser']['user_id']] = $referred_users_item_count['0']['item_count'];
        foreach($users as &$user) $user['User']['item_count'] = ($new_count[$user['User']['id']]) ? $new_count[$user['User']['id']] : 0;
        $this->User->recursive = 2;
    ...
    public function my_api($user_id = null)
    {
        if (is_null($user_id)) {
            throw new NotFoundException(__l('Invalid request'));
        }
    ...
    public function admin_deduct_fund($id = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $item = new Item;
            $item->validate = array_merge($this->User->validate, $this->User->Transaction->validate);
        }
        $this->set('user', $user);
    }
    // <-- For iPhone App code
    ...
    public function index()
    {
        ...
        if (!empty($this->request->params['named']['interest_id'])) {
            $userInterestFollowers = $this->User->UserInterest->UserInterestFollwer->find('all', array(
                'conditions' => array(
                    'UserInterestFollwer.user_interest_id' => $this->request->params['named']['interest_id']
                ) ,
                'recursive' => 0
            ));
            $user_ids = array();
            foreach($userInterestFollowers as $userInterestFollower) {
                $user_ids[] = $userInterestFollower['UserInterestFollwer']['user_id'];
            }
            $conditions['User.id'] = $user_ids;
        }
		if (!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='home') {
			$conditions['User.is_home'] = 1;
		}
		$this->paginate = array(
			'conditions' => $conditions,
			'contain' => array(
