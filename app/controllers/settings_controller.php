    ...
    public function admin_edit($category_id = 1)
    {
        ...
        if (!empty($this->request->data)) {
            if (Configure::read('site.is_admin_settings_enabled')) {
				...
				$validate['error'] = '';
                if (!empty($this->data['Setting']['332']['name'])) {
                    if (stristr(substr($this->data['Setting']['332']['name'], -1, 1) , '/') === FALSE) {
                        $validate['error'] = __l('This is image base URL should not trailing slash');
                        $this->Setting->validationErrors[332]['name'] = __l('This is image base URL should not trailing slash');
                    }
                }
                if (!empty($this->data['Setting']['333']['name'])) {
                    if (stristr(substr($this->data['Setting']['333']['name'], -1, 1) , '/') === FALSE) {
                        $validate['error'] = __l('This is css base URL should have trailing slash');
                        $this->Setting->validationErrors[333]['name'] = __l('This is css base URL should have trailing slash');
                    }
                }
                if (!empty($this->data['Setting']['334']['name'])) {
                    if (stristr(substr($this->data['Setting']['334']['name'], -1, 1) , '/') === FALSE) {
                        $validate['error'] = __l('This is JS base URL should have trailing slash');
                        $this->Setting->validationErrors[334]['name'] = __l('This is JS base URL should have trailing slash');
                    }
                }
				if(empty($validate['error'])) {
                foreach($this->request->data['Setting'] as $id => $value) {
                    $settings['Setting']['id'] = $id;
                    if ($id == '290') { // Writing default city name in cache.
                    ...
                    if ($id == '291') { // Writing conversion currency details in cache
                        Cache::delete('site_paypal_conversion_currency');
                        Cache::delete('site_paypal_conversion_currency_rate');
                        Cache::delete('site_supported_currencies');
                        Cache::delete('site_authorizenet_conversion_currency');
		...
		if ($category_id == 16) { //  module manager that
			$conditions['Setting.name'] = array('affiliate.is_enabled', 'referral.referral_enable', 'friend.is_enabled', 'charity.is_enabled', 'discussions.discussions_enable');
		} else {
		...
		$is_module = false;
		$active_module = true;
		if(in_array($category_id, array(ConstModule::Affiliate, ConstModule::Charity, ConstModule::Friends, ConstModule::Referral) )){
			$is_module = true;
			foreach($settings as $setting){
				if(in_array($setting['Setting']['id'], array(ConstModuleEnableFields::Affiliate, ConstModuleEnableFields::Charity, ConstModuleEnableFields::Friends, ConstModuleEnableFields::Referral, ConstModuleEnableFields::Discussion) )){
        ...
        $fb_login_url = Router::url(array('controller' => 'settings', 'action' => 'update_facebook'), true);
        $tw_login_url = Router::url(array('controller' => 'settings', 'action' => 'update_twitter'), true);
        $fs_login_url = Router::url(array('controller' => 'settings', 'action' => 'admin_update_foursquare'), true);
