...
<?php  echo $this->element('mail-search', array('cache' => array('time' => Configure::read('site.element_cache')))); ?>
...
<?php
        echo $this->Html->link(__l('Star') , array(
            ...
            'class' => "change-star-unstar {'message':'Message has been starred'}"
));
?>
			</span>
            <span class="c1">
<?php
        if ($message['Message']['is_sender'] == 1) :
            ...
            echo $this->Html->link($text->truncate($message['OtherUser']['username'], 16, '') , '#', array());
        endif;
?>
			</span>
		</td>
...
<?php
        echo $this->Html->link($text->truncate(($message['MessageContent']['subject'] . ' - ' . substr($message['MessageContent']['message'], 0, Configure::read('messages.content_length'))) , 60, '...') , array(
            'controller' => 'messages',
            'action' => 'view',
            $message['Message']['hash']
        ) , array());
