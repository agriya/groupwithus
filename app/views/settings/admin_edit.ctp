...
<?php
	...
	if (!empty($settings)):
    	...
    	foreach ($settings as $setting):
				...
				?>
<?php				if(in_array( $setting['Setting']['id'], array(239, 234, 184, 192, 194, 311, 313, 315) ) ) : ?>
                     
                        <h3>
                           <?php echo (in_array($setting['Setting']['id'], array(239, 192, 311)))? __l('Application Info') : ''; ?>
                           <?php echo (in_array($setting['Setting']['id'], array(234, 194, 313)))? __l('Credentials') : ''; ?>
                           <?php echo (in_array($setting['Setting']['id'], array(184, 311, 315)))? __l('Other Info') : ''; ?>
                        </h3>
						<?php if(in_array( $setting['Setting']['id'], array(234, 194, 313))):?>
                            <div class=" info-details">
                                <?php 
                                    if($setting['Setting']['id'] == 234) : 
...
<?php 				endif; ?>                        
				...
				<?php
					if($setting['Setting']['name'] == 'site.is_ssl_for_item_buy_enabled' && !($ssl_enable)){
						$options['disabled'] = 'disabled';
					}
				?>
				<?php
				if($setting['Setting']['name'] == 'affiliate.commission_on_every_item_purchase'):
				?>
				...
				<?php
					...
					if(in_array($setting['Setting']['name'], array('user.referral_item_buy_time', 'user.referral_cookie_expire_time', 'affiliate.referral_cookie_expire_time'))):
						$options['after'] = __l('hrs') . '<span class="info">' . $setting['Setting']['description'] . '</span>';
					endif;
					if( in_array( $setting['Setting']['name'], array('wallet.min_wallet_amount', 'wallet.max_wallet_amount', 'user.minimum_withdraw_amount', 'user.maximum_withdraw_amount', 'merchant.minimum_withdraw_amount', 'merchant.maximum_withdraw_amount', 'affiliate.payment_threshold_for_threshold_limit_reach', 'user.referral_amount', 'referral.refund_amount'))):
						$options['after'] = $currency['conv_currency_symbol']. '<span class="info">' . $setting['Setting']['description'] . '</span>';
					endif;
					...
					$findReplace = array(
								...
								'##MERCHATADD##' => $this->Html->link('Add', Router::url('/', true).$this->request->params['named']['city'].'/admin/merchants/add', array('title' => __l('Merchant add'))),
								'##MERCHANTEDIT##' => $this->Html->link('Edit', '#', array('title' => __l('Merchant edit'))),
					...
					unset($options);
					if (in_array($setting['Setting']['id'], array(235, 195, 314))) {
					?>
                        </div>
                        </div>
					<?php
