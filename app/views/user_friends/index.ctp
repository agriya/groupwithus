...
<?php
	if ($status == ConstUserFriendStatus::Approved) {
		echo $this->Html->link(__l('Reject '), array('action'=>'reject', $userFriend['User']['username']), array('class' => 'reject', 'title' => __l('Reject')));
	}
	if ($status == ConstUserFriendStatus::Pending) {
		echo $this->Html->link(__l('Accept '), array('action'=>'accept', $userFriend['User']['username'], 'received'), array('class' => 'accept', 'title' => __l('Accept')));
		echo $this->Html->link(__l('Reject '), array('action'=>'reject', $userFriend['User']['username'], 'received'), array('class' => 'reject', 'title' => __l('Reject')));
	}
	if ($status == ConstUserFriendStatus::Rejected) {
		echo $this->Html->link(__l('Remove '), array('action'=>'remove', $userFriend['User']['username'], 'received'), array('class' => 'remove', 'title' => __l('Remove')));
	}
?>
