...
<div class="clearfix">
        <div class="grid_15 pass-list-block omega alpha">
  <ul class="pass-chart clearfix">
        ...
        <?php $class = (($this->request->params['controller'] == 'item_users') && (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'all') && empty($this->request->params['named']['gift_user']))) ? ' class="active"' : '';?>
    		<li <?php echo $class; ?>><?php echo $this->Html->link(sprintf(__l('All (%s)'),$all_items),array('controller'=> 'item_users','item_id'=>$item_id, 'action'=>'index','type' => 'all'),array('title' => 'All-'.$all_items)); ?></li>
    	<?php $class = (($this->request->params['controller'] == 'item_users') && (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'all') && !empty($this->request->params['named']['gift_user']))) ? ' class="active"' : '';?>
            <li <?php echo $class; ?>><div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Gifted (%s)'),$gift_items),array('controller'=> 'item_users','item_id'=>$item_id, 'action'=>'index','type' => 'all','gift_user'=>1),array('title' => 'Gifted-'.$gift_items)); ?></div></li>
 		</ul>
 		</div>
 		</div>
		<?php } ?>
		<?php if (($this->Auth->user('user_type_id') == ConstUserTypes::Merchant) && (!empty($this->request->params['named']['item_id']) && (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'available' || $this->request->params['named']['type'] == 'used')) ||  (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass'))) { ?>
		...
		<?php } ?>
		<?php
			echo $this->Form->create('ItemUser' , array('class' => 'normal js-ajax-form','action' => 'update'));
		...
		<table class="list">
			<tr>
				...
				<?php } ?>
                <?php if (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'available' || $this->request->params['named']['type'] == 'used') || (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass')) { ?>
					<th rowspan="2" class="actions"><?php echo __l('Action');?></th>
				<?php } ?>
			...
			<?php endif;?>	
			<?php if(empty($this->request->params['named']['type'])):?>
                    <th><div class="js-pagination"><?php echo __l('Own');?></div></th>
					<th><div class="js-pagination"><?php echo __l('Gift');?></div></th>
			<?php endif;?>	
			</tr>		
					<?php
				if (!empty($itemUsers)):
			...
			<tr<?php echo $class;?>>
				...
				<?php if (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'available' || $this->request->params['named']['type'] == 'used') || (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass')) { ?>
					...
					<?php if ((!empty($pass_find_id) && in_array($itemUser['ItemUserPass'][0]['id'], $pass_find_id)) || (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'available' && empty($itemUser['ItemUserPass'][0]['is_used'])) || (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'used' && !empty($itemUser['ItemUserPass'][0]['is_used'])) ||  (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass')) { ?>
									
											<?php
                                        	$uselink = Router::url(array('controller' => 'item_user_passes', 'action' => 'pass_update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'],'use'), true);
                                        	$undolink = Router::url(array('controller' => 'item_user_passes', 'action' => 'pass_update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'],'undo'), true);

												if($itemUser['ItemUserPass'][0]['is_used'] == 1) {
													$class = 'used';
											...
											?>
								<?php if(empty($this->request->params['named']['item_id'])) { ?>
											<li>	<?php echo $this->Html->link(__l('View Pass'),array('controller' => 'item_users', 'action' => 'view',$itemUser['ItemUser']['id'],'pass_id' => $itemUser['ItemUserPass'][0]['id'],'admin' => false),array('title' => __l('View Pass'), 'class'=>'js-thickbox','target' => '_blank', 'class'=>'view-icon js-thickbox'));?></li>
											<?php } ?>
											<?php if(empty($this->request->params['named']['item_id'])) { ?>
											<li><?php echo $this->Html->link(__l('Print'),array('controller' => 'item_users', 'action' => 'view',$itemUser['ItemUser']['id'],'pass_id' => $itemUser['ItemUserPass'][0]['id'],'type' => 'print'),array('target'=>'_blank', 'title' => __l('Print'), 'class'=>'print-icon'));?></li>
											<?php } ?>                                     
											<?php
												...
												if ((!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='available') || !empty($this->request->params['named']['item_id'])) {
													if (!empty($itemUser['ItemUserPass'][0]['is_used']) && $itemUser['Item']['merchant_id'] == $user['Merchant']['id']) {
												?>
                                                 	<?php
                                                            if(!empty($itemUser['ItemUserPass'][0]['is_used'])){
                                                                $use_now = __l('Used');
																$types = Configure::read('item.item_pass_used_type');
                                                                																
																if($types == 'click'){ ?>
													     	<li>
													     	<span class="<?php echo 'status-'.$itemUser['ItemUserPass'][0]['is_used']?>">
																<?php echo $use_now;
                                                                	echo $this->Html->link(__l('Undo'), array('controller' => 'item_user_passes', 'action' => 'update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'],'is_used'), array('class' => $class.' js-update-status','title' => $statusMessage)); ?>
                                                                </span>
                                                                </li>
                                                                <?php }
																...
																if($types == 'submit' && $this->Auth->user('user_type_id') == ConstUserTypes::Merchant){
																	...
																	if($itemUser['Item']['merchant_id'] == $user['Merchant']['id']) {
																		$confirmation_message =  "{'divClass':'js-merchant-confirmation', 'uselink':'".$uselink."', 'undolink':'".$undolink."', 'code_get':'".'ItemUserPass'.$itemUser['ItemUserPass'][0]['id'].$code_type."', 'process':'undo'}";
																	} else {
																		$confirmation_message = "{'divClass':'js-user-confirmation', 'uselink':'".$uselink."', 'undolink':'".$undolink."', 'code_get':'".'ItemUserPass'.$itemUser['ItemUserPass'][0]['id'].$code_type."', 'process':'undo'}";
																	} ?>
														        <li>
														        <span class="<?php echo 'status-'.$itemUser['ItemUserPass'][0]['is_used']?>">
																<?php echo $this->Html->link(__l('Undo'), array('controller' => 'item_user_passes', 'action' => 'pass_update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'],'undo'), array('class' => $class.' '.$confirmation_message.' js-pass-update-status','title' => $statusMessage)); ?>
																</span>
																</li>
															<?php }
                                                            }else{
																if(!empty($itemUser['Item']['event_date'])):
                                                                  ...
                                                                  <li>
													        	<span class="<?php echo 'status-'.$itemUser['ItemUserPass'][0]['is_used']?>">
																	<?php echo $this->Html->link($use_now, array('controller' => 'item_user_passes', 'action' => 'update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'],'is_used'), array('class' => $class.' '.$confirmation_message.' js-update-status','title' => $statusMessage)); ?>
																	</span>
																	</li> <?php
																	endif;
																endif;
...
?>													<?php } ?>
													<?php if ($class == 'not-used')  { ?>
														...
														<?php
                                                           ...
                                                           <li>
													     	<span class="<?php echo 'status-'.$itemUser['ItemUserPass'][0]['is_used']?>">
                                                                <?php echo $use_now;
															    echo $this->Html->link(__l('Undo'), array('controller' => 'item_user_passes', 'action' => 'update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'], 'is_used'), array('class' => $class.' '.$confirmation_message.' js-update-status', 'title' => $statusMessage));
															    ?>
															    </span>
															    </li>
														<?php }else {  ?>
																	...
																	if((date('Y-m-d H:i:s') >= _formatDate('Y-m-d H:i:s', strtotime($itemUser['Item']['event_date'])))):
					                 								...
					                 								<?php
																		...
																		if($types == 'submit' && $this->Auth->user('user_type_id') == ConstUserTypes::Merchant){
																	       ...
																	       <li>
													                    	<span class="<?php echo 'status-'.$itemUser['ItemUserPass'][0]['is_used']?>">
																			<?php echo $this->Html->link(__l('Use Now'), array('controller' => 'item_user_passes', 'action' => 'pass_update_status', $itemUser['ItemUser']['id'], 'pass_id' => $itemUser['ItemUserPass'][0]['id'],'use'), array('class' => $class.' '.$confirmation_message.' js-pass-update-status','title' => $statusMessage)); ?>
														             	  </span>
																	    	</li>
            												      	<?php }	?>
																			<?php endif;
				...
				<?php if (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'available' || $this->request->params['named']['type'] == 'used') ||  (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass') ):?>
						...
						<?php if (empty($itemUser['ItemUser']['is_gift']) || (!empty($itemUser['ItemUser']['is_gift']) && $itemUser['ItemUser']['gift_email'] == $this->Auth->user('email')) || !empty($this->request->params['named']['item_id'])):?>
					       <ul class="action-link pass-code clearfix">
									<?php if ((!empty($pass_find_id) && in_array($itemUser['ItemUserPass'][0]['id'], $pass_find_id)) || (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'available' && empty($itemUser['ItemUserPass'][0]['is_used'])) || (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'used' && !empty($itemUser['ItemUserPass'][0]['is_used'])) ||  (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass')) { ?>
										<li class="clearfix">
                                        	<?php if(Configure::read('item.item_pass_used_type') == 'click'){ ?>
														<span class="pass-code"><?php echo $itemUser['ItemUserPass'][0]['unique_pass_code']; ?></span>
                                            <?php } else { ?>
												 <?php  
												 	if( $this->Auth->user('user_type_id') == ConstUserTypes::Merchant ){
														if( Configure::read('item.item_pass_code_show_type') == 'top') {
															 echo $this->Form->input('ItemUserPass.'.$itemUser['ItemUserPass'][0]['id'].'.unique_pass_code', array('type' => 'text', 'label' => false, 'div' => false)); 
														}
														else{
												?>
	                                                		<span class="pass-code"><?php echo $itemUser['ItemUserPass'][0]['unique_pass_code']; ?></span>
					...
					<td>
						...
						<?php if (empty($itemUser['ItemUser']['is_gift']) || (!empty($itemUser['ItemUser']['is_gift']) && $itemUser['ItemUser']['gift_email'] == $this->Auth->user('email')) || !empty($this->request->params['named']['item_id'])):?>
					          <ul class="pass-code clearfix">
									<?php if ((!empty($pass_find_id) && in_array($itemUser['ItemUserPass'][0]['id'], $pass_find_id)) || (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'available' && empty($itemUser['ItemUserPass'][0]['is_used'])) || (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'used' && !empty($itemUser['ItemUserPass'][0]['is_used'])) || (!empty($show_pass_code) && $this->request->params['named']['item_user_view'] == 'pass')) { ?>
										<li class="clearfix">
                                        <?php if(Configure::read('item.item_pass_used_type') == 'click'){ ?>
														<span class="pass-code"><?php echo $itemUser['ItemUserPass'][0]['pass_code']; ?></span>
                                            <?php } else { ?>
												 <?php  
												 	if( $this->Auth->user('user_type_id') == ConstUserTypes::Merchant ){
														if( Configure::read('item.item_pass_code_show_type') == 'bottom') {
															 echo $this->Form->input('ItemUserPass.'.$itemUser['ItemUserPass'][0]['id'].'.pass_code', array('type' => 'text', 'label' => false, 'div' => false)); 
														}
														else{
												?>
                                                		<span class="pass-code"><?php echo $itemUser['ItemUserPass'][0]['pass_code']; ?></span>
					...
					<td>
						<?php if(!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='available'):?>
							<?php echo $this->Html->cInt(1 - $itemUser['ItemUser']['item_user_pass_count']);?>
						...
						<?php elseif(!empty($this->request->params['named']['type']) && ($this->request->params['named']['type']=='all' && empty($this->request->params['named']['gift_user']))):?>
							<?php echo $itemUser['ItemUser']['quantity'];?>
						<?php elseif(!empty($this->request->params['named']['type']) && ($this->request->params['named']['type']=='all' && !empty($this->request->params['named']['gift_user']))):?>
							<?php echo $itemUser['ItemUser']['quantity'] -1;?>						<?php else:?>
							<?php echo 1;?>
						<?php endif;?>
					</td>	
						<?php if(empty($this->request->params['named']['type'])):?>
					<td>
						...
						<?php elseif(!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='used'):?>
							<?php echo $itemUser['ItemUser']['item_user_pass_count'];?>
						<?php else:?>
							<?php echo $itemUser['ItemUser']['quantity'] -1;?>
						<?php endif;?>
					</td>
					<?php endif;?>
					<?php if(Configure::read('charity.is_enabled') == 1):?>
					...
					if(empty($itemUser['CharitiesItemUser']['Charity']['name']) && $itemUser['CharitiesItemUser']['amount'] == 0.00):
						...
						echo Configure::read('site.currency') . $this->Html->cCurrency($itemUser['CharitiesItemUser']['amount']); echo (!empty($itemUser['CharitiesItemUser']['Charity']['name']))? ' '.'for'.' '.$itemUser['CharitiesItemUser']['Charity']['name']:'';?>
                    <?php endif ?>
                    </td>		
					<?php endif ?>
				</tr>
	...
	?>
        ...
        <?php if (!empty($itemUsers) && !empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'available' || $this->request->params['named']['type'] == 'used')):?>
            <?php if(!empty($itemUser['Item']['item_status_id']) && ($itemUser['Item']['item_status_id'] != ConstItemStatus::PendingApproval || $itemUser['Item']['item_status_id'] != ConstItemStatus::Expired) && ((!empty($this->request->params['named']['type']) && ($this->request->params['named']['type']!='gifted_items')))){?>
		...
		<?php endif;?>
        <?php if (!empty($itemUsers) && !empty($this->request->params['named']['item_user_view']) && $this->request->params['named']['item_user_view'] == 'pass' && ($this->Auth->user('user_type_id') == ConstUserTypes::Merchant) && (Configure::read('item.item_pass_used_type') == 'submit')):?>
