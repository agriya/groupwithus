...
<div class="mailChimpLists index">
	<div class="page-info">
		<?php echo __l('MailChimp module is currently enabled. You can disable or configure it from').' '.$this->Html->link(__l('Settings'), array('controller' => 'settings', 'action' => 'edit', 14, '#' => 'MailChimp'), array('target' => '_blank')). __l(' page');?>
	</div>
	...
	<div class="form">
		<?php echo $this->Form->create('MailChimpList', array('action' => 'admin_index', 'class' => 'normal'));?>
			...
			<div class="overflow-block">
				<table class="list">
					<tr>     	
						<th><?php echo __l('Interests');?></th>
						<th class="dl"><?php echo __l('List ID');?><span class="info"><?php echo __l('It is the unique ID of the mailchimp list');?></span></th>
					</tr>			
					<?php  foreach($interest_mail_chimp_lists as $interest_mail_chimp_list) { ?>
						<tr>
							<td><?php echo $this->Html->cText($interest_mail_chimp_list['UserInterest']['name'],false); ?></td>
							<td class="dl">
								<?php
									$id = $list_id = '';
									if(!empty($interest_mail_chimp_list['MailChimpList']['id'])){
										$id = $interest_mail_chimp_list['MailChimpList']['id'];						
									}
									if(!empty($interest_mail_chimp_list['MailChimpList']['list_id'])){
										$list_id = $interest_mail_chimp_list['MailChimpList']['list_id'];
									}
									echo $this->Form->input('MailChimpList.'.$interest_mail_chimp_list['UserInterest']['id'].'.id', array('value' => $id, 'label' => false, 'type' => 'hidden'));
									echo $this->Form->input('MailChimpList.'.$interest_mail_chimp_list['UserInterest']['id'].'.user_interest_id', array('value' => $interest_mail_chimp_list['UserInterest']['id'], 'label' => false, 'type' => 'hidden'));
									echo $this->Form->input('MailChimpList.'.$interest_mail_chimp_list['UserInterest']['id'].'.list_id', array('value' => $list_id, 'label' => false, 'type' => 'text'));
									?>
							</td>
						</tr>
					<?php } ?>
