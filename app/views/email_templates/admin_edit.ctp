...
<?php
	...
	echo $this->Form->input('email_content_ja', array('label' => __l('Email Content (Japanese)'),'type' =>'textarea', 'class' => 'js-email-content email-content js-editor', 'id' => 'EmailTemplateEmailContent'.$this->request->data['EmailTemplate']['id'], 'info' => $this->Html->cText($this->request->data['EmailTemplate']['email_variables'], false)));
    ?>
