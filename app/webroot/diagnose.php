...
$facebook_settings_id = 54;
$twitter_settings_id = 55;
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<title>GroupWithUs - Diagnostic Tool</title>
<style type="text/css">
body{
	color:#4c4c4c;
	line-height:18px;
...
table.list tr th.item-name,table.list tr td.item-name {
	width:150px;
	white-space:wrap;
}
...
table.list tr td.item-name,table.list tr th.item-name {
	width:220px;
}
...
$master_db = _db_check('master',$facebook_settings_id,$twitter_settings_id);
$default_db = _db_check('default',$facebook_settings_id,$twitter_settings_id);
...
<?php
//---------------- Default table -------------------------------------------------------
function _db_check($database = 'default',$facebook_settings_id,$twitter_settings_id)
{
    ...
    if ($link) {
        ...
        if ($db_selected) {
            ...
            foreach($db_info['payment_gateway']['gateway'] as $key => &$gateway) {
                foreach($db_info['payment_gateway']['gateway_settings'] as &$settings) {
                    if ($gateway['id'] == $settings['payment_gateway_id']) {
                        if (($settings['key'] != 'is_enable_for_buy_a_item' && $settings['key'] != 'is_enable_for_add_to_wallet')) {
            ...
            $result = mysql_query("select * from settings where setting_category_parent_id in (1, 2, 11, 14)", $link);
            $db_info['payment_gateway']['error'] = 'None';
            if (!$result) {
                echo $db_info['payment_gateway']['error'] = mysql_error();
            } else {
                $db_info['settings'][$twitter_settings_id] = $db_info['settings'][$facebook_settings_id] = array(
                    'Mandatory' => array() ,
                    'Credentials' => array() ,
                    'Not Mandatory' => array()
                );
                ...
                while ($row = mysql_fetch_assoc($result)) {
                    ...
                    } elseif ($row['setting_category_id'] == $facebook_settings_id || $row['setting_category_id'] == $twitter_settings_id) {
                        $db_info['settings'][$row['setting_category_id']]['Mandatory'][] = array(
							'name' => $row['name'],
                            'label' => $row['label'],
                            'value' => $row['value']
...
if ($_GET['action'] == 'post' && $_GET['to'] != '' && $_GET['to'] == 'facebook') {
    if (!empty($master_db['settings'][$facebook_settings_id]['Mandatory'][0]['value']) && !empty($master_db['settings'][$facebook_settings_id]['Mandatory'][2]['value']) && !empty($master_db['settings'][$facebook_settings_id]['Mandatory'][4]['value']) && !empty($master_db['settings'][$facebook_settings_id]['Mandatory'][7]['value'])) {
        $site = $master_db['settings']['site'][0];
        $app_id = $master_db['settings'][$facebook_settings_id]['Mandatory'][0]['value'];
        $secret_key = $master_db['settings'][$facebook_settings_id]['Mandatory'][4]['value'];
        $fb_access_token = $master_db['settings'][$facebook_settings_id]['Mandatory'][2]['value'];
        $fb_page_id = $master_db['settings'][$facebook_settings_id]['Mandatory'][7]['value'];
...
if ($_GET['action'] == 'post' && $_GET['to'] != '' && $_GET['to'] == 'twitter') {
    if (!empty($master_db['settings'][$twitter_settings_id]['Mandatory'][0]['value']) && !empty($master_db['settings'][$twitter_settings_id]['Mandatory'][1]['value']) && !empty($master_db['settings'][$twitter_settings_id]['Mandatory'][5]['value']) && !empty($master_db['settings'][$twitter_settings_id]['Mandatory'][6]['value'])) {
        function post_tweet($tweet_text, $master_db, $twitter_settings_id)
        {
            ...
            $connection = new tmhOAuth(array(
                ...
                'user_token' => $master_db['settings'][$twitter_settings_id]['Mandatory'][6]['value'],
                'user_secret' => $master_db['settings'][$twitter_settings_id]['Mandatory'][5]['value'],
            ));
            // Make the API call
...
<h1>GroupWithUs Diagnostic Tool</h1>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=groupwithus-install#setting_up_files" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=groupwithus-install#setting_up_files</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=groupwithus-install#setting_up_database" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=groupwithus-install#setting_up_database</a>
	</span>
</p>
...
<p class="info-details">
    ...
    <span>
		...
		<br/>For more info: <a href="http://dev1products.dev.agriya.com/doku.php?id=groupwithus-install#manage_site_settings" target="_blank">http://dev1products.dev.agriya.com/doku.php?id=groupwithus-install#manage_site_settings</a>
	</span>
</p>
<?php
...
<table border="2" class="list">
  ...
  <tr>
    ...
    <th>Active?</th>
    <th>Live mode?</th>
    <th>Enable for Item Purchase?</th>
...
?>
  <tr>
    ...
    <td><?php
        echo ($gateway['is_enable_for_buy_a_item']) ? 'Yes' : 'No'; ?></td>
    <td><?php
...
?>
	...
	<?php
		...
		{
			$setting['Mandatory'][11] = $setting['Mandatory'][2];
		...
		{
			$setting['Mandatory'][11] = $setting['Mandatory'][5];
			$setting['Mandatory'][12] = $setting['Mandatory'][6];
            unset($setting['Mandatory'][5]);
			...
			foreach($setting as $data => $step) {
				...
				elseif($key == $facebook_settings_id || $key == $twitter_settings_id)
				{
					$count = count($step)+1;
				} else {
					$count = count($step);
...
<?php							
						...
						if($mantry['name'] == 'subscription.urbanairship_app_key')
						{
?>
							<tr>
							<td colspan="2">
                            <div class="cred_detail">
                            The following 3 settings will be used only if "Push Notification Server" is set as Urbanairship.
                            </div>
                            </td>
                            </tr>
<?php							
						}
						if($mantry['name'] == 'Live Certification')
						{
?>
							<tr>
							<td colspan="2">
                            <div class="cred_detail">
                            The following setting will be used only if "Push Notification Server" is set as Site.
                            </div>
                            </td>
                            </tr>
<?php							
...
?>
                            ...
                            $settings_check_one_zero = array(
                                'site.is_in_prelaunch_mode',
                                'site.maintenance_mode',
                                'facebook.enable_facebook_post_open_item',
                                'twitter.prompt_for_email_after_register',
                                'twitter.enable_twitter_post_open_item',
                            );
                            $secure_key_value = array(
								...
								'invite.yahoo_secret_key',
								'friend.msn_secret',
								'subscription.urbanairship_secret_key'
                            );
