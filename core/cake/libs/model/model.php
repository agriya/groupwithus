...
// From Siva. For Sphinx. @todo better naming and indentation
...
// <!-- timezone related code
		if (Configure::read('site.timezone_offset')) {
			foreach($this->data[$this->alias] as $field => $value) {
				if (preg_match('/_date$/', $field) || preg_match('/_time$/', $field)) {
					$hour_var = split(':', $value);
					if (!empty($hour_var[1])) {
						$this->data[$this->alias][$field] = gmdate('Y-m-d H:i:s', strtotime($this->data[$this->alias][$field]));
					} else {
						$this->data[$this->alias][$field] = gmdate('Y-m-d', strtotime($this->data[$this->alias][$field]));
					}
				}
			}
		}
...
// <!-- timezone related code
				if (Configure::read('site.timezone_offset')) {
					if (!array_key_exists('format', $colType)) {
						$time = strtotime('now GMT');
					} else {
						$time = gmdate('Y-m-d H:i:s', strtotime($colType['formatter']($colType['format'])));
					}
				} else {
					if (!array_key_exists('format', $colType)) {
						$time = strtotime('now');
...
// <!-- timezone related code
		if (Configure::read('site.timezone_offset')) {
			if(!empty($fields)) {
				$tempFields = $fields;
				foreach($tempFields as $field => $value) {
					if (preg_match('/_date$/', $field) || preg_match('/_time$/', $field)) {
						$hour_var = split(':', $value);
						$value = str_replace('\'', '', $value);
						$value = str_replace('"', '', $value);
						if (!empty($hour_var[1])) {
							$fields[$field] = '\'' . gmdate('Y-m-d H:i:s', strtotime($value)) . '\'';
						} else {
							$fields[$field] = '\'' . gmdate('Y-m-d', strtotime($value)) . '\'';
						}
					}
				}
			}
...
// From Siva, for sphinx -->
        if (isset($query['search'])) {
            $sphinxSearch = false;
            if ($sphinxSearch) {
                App::import('Behavior', 'Sphinx');
                $sphinxObj = new SphinxBehavior;
                $sphinxObj->setup($this);
                $return = $sphinxObj->beforeFind($this, $query, array(
                    'break' => true,
                    'breakOn' => false,
                    'modParams' => true
                ));
                ...
                // Getting fields list from this->model
                $_schema = $this->schema();
                $fields = array();
                if (!empty($_schema)) {
                    foreach($_schema as $key => $value) {
                        if ($value['type'] == 'string' || $value['type'] == 'text') {
                            $fields[] = '' . $this->name . '.' . $key . '';
                        }
                    }
                }
                // Getting fields list from association models
                // Only hasOne and belongsTo and only type => string, text
                $associations = $this->getAssociated();
                if (!empty($associations)) {
                    foreach($associations as $model => $relationship) {
                        if ($relationship == 'belongsTo' || $relationship == 'hasOne') {
							foreach($this->{$model}->schema() as $key => $value) {
								if ($value['type'] == 'string' || $value['type'] == 'text') {
									$fields[] = '' . $this->{$model}->name . '.' . $key . '';
								}
							}
                        }
                    }
                ...
                // build 'OR' query
                $keywordsArray = explode(' ', $query['search']);
                $searchCondition['OR'] = array();
                if (!empty($fields)) {
                    foreach($fields as $field) {
                        foreach($keywordsArray as $keyword) {
                            if (count($keywordsArray) > 1) {
                                $searchCondition['OR'][][$field . ' LIKE '] = '%' . $keyword . '%';
                            } else {
                                $searchCondition['OR'][$field . ' LIKE '] = '%' . $keyword . '%';
                            }
                        }
                ...
                if (!empty($query['conditions'])) {
                    foreach($query['conditions'] as $key => $value) {
                        if (!is_array($query['conditions'][$key]) && strtoupper($query['conditions'][$key]) == 'OR') {
                            $query['conditions'][strtoupper($key) ] = $query['conditions'][$key];
                            unset($query['conditions'][$key]);
                        }
                    }
                }
                $query['conditions']['OR'] = isset($query['conditions']['OR']) ? array_merge($searchCondition['OR'], $query['conditions']['OR']) : $searchCondition['OR'];
            }
        }
// <--
		...
		// Siva // to unset unwanted model relations for count query
		if ($type == 'count' && (!isset($query['recursive']) || $query['recursive'] == 0)) {
			$countConditionModel = array();
			if (!empty($query['conditions'])) {
				$countConditionModel = $this->_findCountModel($query['conditions'], $countConditionModel);
			}
			$_associations = $this->associations();
			foreach ($_associations as $relationType) {
				$this->__backAssociation[$relationType] = $this->{$relationType};
				foreach ($this->{$relationType} as $assoc => $assocData) {
					if (!in_array($assoc, $countConditionModel)) {
						unset($this->{$relationType}[$assoc]);
					}
				}
			}
		}
	...
	function _findCountModel($conditions, $countConditionModel) {
		foreach($conditions as $k => $v) {
			if (is_array($v)) {
				$countConditionModel = $this->_findCountModel($v, $countConditionModel);
			} else {
				if (preg_match('/^([\w-]+)\((.*)\)$/', $k, $matches)) {
					$k = $matches[2];
				}
				$conditionModelArr = explode('.', $k);
				if (sizeof($conditionModelArr) >= 2) {
					$conditionModelArr[0] = str_replace('`', '', $conditionModelArr[0]);
					$countConditionModel[] = $conditionModelArr[0];
				}
			}
		}
		$countConditionModel = array_unique($countConditionModel);
 ...
 * @access protected
 */
	function _clearPermanentCache() {
		if (PERMANENT_CACHE_CHECK === true) {
			$assoc[] = $this->alias;
			foreach ($this->__associations as $key => $association) {
				foreach ($this->$association as $key => $className) {
			...
			clearPermanentCache(array_unique($assoc));
			return true;
		}
	}
